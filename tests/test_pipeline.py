...
def test_cpython_pymalloc_arena_fuzzing():
    import importlib.util
    from fuzz_lab82_cpython_pymalloc_arena.pymalloc_runner import run_pymalloc_target

    mut_path = "fuzz_lab82_cpython_pymalloc_arena/ai_mutator_pymalloc.py"
    if not os.path.exists(mut_path):
        pytest.skip("Lab 82 mutator not found")

    spec = importlib.util.spec_from_file_location("ai_mutator_pymalloc", mut_path)
    mut = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mut)
    ...
    target_py = "fuzz_lab82_cpython_pymalloc_arena/target.py"
    crash_file = "fuzz_lab82_cpython_pymalloc_arena/in/crash_pymalloc.bin"
    if os.path.exists(target_py) and os.path.exists(crash_file):
...
def test_python_memoryview_invalidation_fuzzing():
    import importlib.util
    from fuzz_lab87_python_memoryview_invalidation.memoryview_runner import run_memoryview_target

    mut_path = "fuzz_lab87_python_memoryview_invalidation/ai_mutator_memoryview.py"
    if not os.path.exists(mut_path):
        pytest.skip("Lab 87 mutator not found")

    spec = importlib.util.spec_from_file_location("ai_mutator_memoryview", mut_path)
    mut = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mut)
    ...
    assert res[:4] == b"MVW\x00"
    mut.deinit()

    target_py = "fuzz_lab87_python_memoryview_invalidation/target.py"
    crash_file = "fuzz_lab87_python_memoryview_invalidation/in/crash_invalidation.bin"
    if os.path.exists(target_py) and os.path.exists(crash_file):
